...
SUPPORTED_FORMATS = {".csv": {"cls": csv.DictReader, "kwargs": {"delimiter": ";"}, "header": "fieldnames"}}
...
class LotoResult(NamedTuple):
    ...
    def __str__(self) -> str:
        if self.date is not None:
            ...
            date = ""
        return f"tirage={'-'.join(map(str, sorted(self.grid)))}+{'+'.join(map(str, sorted(self.chance)))}{date}"

    def __eq__(self, other: "LotoResult") -> bool:
    ...
    def __ne__(self, other: "LotoResult") -> bool:
        return not (self == other)

    @classmethod
    def from_string(cls, input_string: str) -> "LotoResult":
        try:
            grid, _, chance = input_string.partition("+")
            grid, chance = ([int(i) for i in grid.split("-")], [int(i) for i in chance.split("+")])
        except Exception:
            raise ValueError(f"Invalie format for {input_string}")

        return cls(grid, chance)

def formatter(prefix: str, idx: int):
    return f'{prefix}{idx}'


class LotoFormat(Iterable):
    CHANCE_NB: int
    CHANCE_PREFIX: str
    BOULES_NB: int
    ...
    MAX = 49
    MAX_CHANCE = 10
    WHITELIST_BOULE: Optional[str] = None
    ...
    @classmethod
    def whitelist(cls, key):
        if cls.WHITELIST_BOULE is None:
            return False
        return re.match(cls.WHITELIST_BOULE, key) is not None

    @classmethod
    def is_one(cls, to_compare: LotoResult | Sequence[str]):
        if isinstance(to_compare, LotoResult):
            return len(set(to_compare.grid)) == cls.BOULES_NB and len(set(to_compare.chance)) == cls.CHANCE_NB and all(
                0 < i <= cls.MAX for i in to_compare.grid) and all(0 < i <= cls.MAX_CHANCE for i in to_compare.chance)
        return (
            all(boule in to_compare for boule in cls.boule_keys)
            and all(chance in to_compare for chance in cls.chance_keys)
            and len([
                h for h in to_compare
                if (h.startswith(cls.BOULE_PREFIX) or h.startswith(cls.CHANCE_PREFIX)) and (h not in cls.boule_keys + cls.chance_keys )and not cls.whitelist(h)
            ]) == 0
        )

    def extract_line(self, line: dict[str, Any]) -> Sequence[LotoResult]:
    ...
    @classmethod
    def generate(cls):
        return LotoResult(random.sample(range(1, cls.MAX + 1), k=cls.BOULES_NB), random.sample(range(1, cls.MAX_CHANCE + 1), k=cls.CHANCE_NB))
...
LOTO_FORMATS: dict[str, LotoFormat] = {
...
def find_date_format(getted_date: str):
    if "/" in getted_date:
        yield "%d/%m/%Y"
        yield "%d/%m/%y"
    yield "%Y%m%d"
...
def generate_tirage(results, loto_cls) -> LotoResult:
...
@click.command()
@click.option("-r", "--repository", type=click.Path(exists=True, file_okay=False, path_type=pathlib.Path))
@click.option("-f", "--file", type=click.Path(exists=True, dir_okay=False,
              path_type=pathlib.Path), default=[], multiple=True)
@click.option("-l", "--loto-format", type=click.Choice(LOTO_FORMATS), default="5_boules")
@click.option("-e", "--exist", type=str, default=None)
@click.pass_context
def main(ctx: click.Context, repository: pathlib.Path,
         file: list[pathlib.Path], loto_format: str, exist: Optional[str]) -> None:
    if repository and file:
        raise click.exceptions.UsageError("--repository and --file can't be set simultaneously", ctx=ctx)
    ...
    if any(f.suffix not in SUPPORTED_FORMATS for f in file):
        raise click.exceptions.BadParameter(f"--file must be one of {', '.join(SUPPORTED_FORMATS)}", ctx=ctx)
    ...
    if exist is not None:
        try:
            ...
            raise click.exceptions.BadArgumentUsage(e)
